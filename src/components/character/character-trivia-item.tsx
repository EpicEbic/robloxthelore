...
export function CharacterTriviaItem({ item }: CharacterTriviaItemProps) {
  if (item.toLowerCase().includes("glasses")) {
    return (
      <div className="flex items-center gap-2 break-words whitespace-normal overflow-wrap-anywhere">
        ...
        <span className="break-words whitespace-normal overflow-wrap-anywhere">{item}</span>
      </div>
    );
  ...
  return <div className="break-words whitespace-normal overflow-wrap-anywhere">{item}</div>;
}
