...
export function ComparisonResults({ entry1, entry2, combatStyle1, combatStyle2 }: ComparisonResultsProps) {
  // Calculate power level based on physical stats only
  const calculatePowerLevel = (entry: WikiEntry) => {
    ...
    const maxPossiblePower = 35; // 5 physical stats * 7 max value (Ø = 7)
    
    return Math.min(100, (physicalTotal / maxPossiblePower) * 100);
  };
  ...
  const getStatDisplay = (stat: { label: string; value: number } | undefined, baseStat?: { label: string; value: number }) => {
        ...
        <div className="p-6 border-b border-border/50">
          ...
          <div className="space-y-4">
            <p className="text-sm text-muted-foreground text-center">
              Based on physical stats comparison.
            </p>
          ...
          <div className="p-6 border-b border-border/50">
            ...
            <div className="space-y-6">
              {[
                ...
                const display1 = getStatDisplay(stat1, baseStat1);
                const display2 = getStatDisplay(stat2, baseStat2);
